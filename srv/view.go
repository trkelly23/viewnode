	...
	for _, n := range vnd.Nodes {
		if n.Name != "" {
			...
			for _, p := range n.Pods {
				vnd.PrintoutPodHeader(p)
				if vnd.Config.ShowContainers {
					vnd.PrintoutPodContainers(p)
				}
				fmt.Println()
...
func (vnd ViewNodeData) PrintoutNode(n ViewNode) {
...
func (vnd ViewNodeData) PrintoutPodHeader(p ViewPod) {
...
func (vnd ViewNodeData) PrintoutPodContainers(p ViewPod) {
